...
describe('Promise.all', () => {
  ...
  describe('should create the resolved', () => {
    ...
    test('resolved with a correct value #1', done => {
      Adapter.all([Adapter.resolve(1), Adapter.resolve(2), Adapter.resolve(3)]).then(value => {
    ...
    test('resolved with a correct value #1', done => {
      const date = new Date();
      Adapter.all([1, null, undefined, true, false, {}, [], date, '', Promise.resolve(2)]).then(value => {
  ...
  describe('should create the rejected', () => {
    ...
    test('rejected as expected #1', done => {
      Adapter.all([Adapter.resolve(1), Adapter.resolve(2), Adapter.reject(3)]).catch(error => {
        expect(error).toBe(3);
        done();
      });
    });
