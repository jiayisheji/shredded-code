...
describe('Promise.race', () => {
  ...
  describe('should create the resolved', () => {
    test('resolved with a correct value', done => {
      Adapter.race([Adapter.resolve(1), Adapter.reject(2), Adapter.resolve(3)]).then(value => {
        expect(value).toBe(1);
        done();
      });
    });
    ...
    test('resolved with a correct value #1', done => {
      Adapter.race([1, Adapter.reject(2), Adapter.resolve(3)]).then(value => {
        expect(value).toBe(1);
        done();
      });
    });
  ...
  describe('should create the rejected', () => {
    ...
    test('rejected as expected #2', done => {
      Adapter.race([Adapter.reject(1), Adapter.resolve(2)]).catch(error => {
        expect(error).toBe(1);
        done();
      });
    });
